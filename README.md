# 🚀 DSA LeetCode Journey

A structured repository where I store my LeetCode solutions with multiple approaches for revision and pattern recognition.

---

## 📊 Progress

- Total: 18
- Easy: 16
- Medium: 1
- Hard: 0

---

## 🔥 Recent Problems
<!-- recent -->
- dummy (Unknown, arrays) → https://leetcode.com/problems/dummy/
- Contains Duplicate (Easy, arrays) → https://leetcode.com/problems/contains-duplicate/
- Remove Duplicates from Sorted Array (Easy, two-pointers) → https://leetcode.com/problems/remove-duplicates-from-sorted-array/
- Roman to Integer (Easy, strings) → https://leetcode.com/problems/roman-to-integer/
- Plus One (Easy, arrays) → https://leetcode.com/problems/plus-one/
- Two Sum (Easy, arrays) → https://leetcode.com/problems/two-sum/
- Mirror Distance of an Integer (Easy, math) → https://leetcode.com/problems/mirror-distance-of-an-integer/
- Furthest Point From Origin (Easy, greedy) → https://leetcode.com/problems/furthest-point-from-origin/
- Add Two Integers (Easy, math) → https://leetcode.com/problems/add-two-integers/
- Maximum Number of Words Found in Sentences (Easy, strings) → https://leetcode.com/problems/maximum-number-of-words-found-in-sentences/

---

## 📁 Structure

Each problem contains:

- v1.py → My initial solution  
- v2.py → Optimized solution  
- v3.py → Alternative approach  
- notes.md → Key learnings  

---

➡️ Full list → problems.md
---

## 🧠 Approach Strategy

- Focus on understanding patterns, not just solving problems  
- Compare multiple approaches (v1 → v2 → v3)  
- Maintain notes for revision  
- Build intuition for interviews  

---

## ⚙️ Automation

This repo uses a custom script to:

- Generate problem folders automatically  
- Update `README.md` stats  
- Maintain `problems.md`  
- Track progress  

---

## 🚀 Goal

Build strong problem-solving skills and master DSA through consistency and structured learning.